...
export async function deploy(hre: HardhatRuntimeEnvironment) {
  ...
  const estimatedGas = await Contract.signer.estimateGas(Contract.getDeployTransaction(contractName, contractSymbol));
  ...
  await _saveContractAddresses({ address: contract.address });
...
const _getContract = async (hre: HardhatRuntimeEnvironment) => {
  ...
  try {
    address = JSON.parse(
      fs.readFileSync(path.join(addressesDir, "/", contractFileName + "-contractAddress.json")).toString(),
    )[contractFileName];
  } catch (e) {
